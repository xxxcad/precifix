...
export default async function Page({
  ...
  const [{ canal }, items] = await Promise.all([
    searchParams,
    loadRepricingQueue(),
  ]);
  const allPending = items.filter((item) =>
    ["OPEN", "IN_PROGRESS"].includes(item.status),
  );
  const channels = Array.from(
    new Set(allPending.map((item) => item.marketplace)),
  ).sort();
  const pending = canal
    ? allPending.filter((item) => item.marketplace === canal)
    : allPending;
  const history = items
    ...
    <>
      ...
      <section className="wide-card">
        <div className="card-heading">
          ...
          <strong className="queue-count">{pending.length}</strong>
        </div>
        <nav className="channel-filters">
          <Link className={!canal ? "selected" : ""} href="/reprecificacao">
            Todos
          </Link>
          {channels.map((channel) => (
            <Link
              className={canal === channel ? "selected" : ""}
              href={`/reprecificacao?canal=${encodeURIComponent(channel)}`}
              key={channel}
            >
              {channel}
            </Link>
        ...
        {pending.length ? (
          <RepricingPendingTable items={pending} />
        ) : (
