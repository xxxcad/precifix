...
export async function updateQueueItems(formData: FormData) {
  ...
  redirect(`/reprecificacao?success=${encodeURIComponent(parsed.data.length === 1 ? "Reprecificação marcada como feita" : `${parsed.data.length} reprecificações marcadas como feitas`)}` as Route);
}
