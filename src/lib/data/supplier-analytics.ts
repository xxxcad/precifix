...
export const calculateProductScenarioAverage = (product:SupplierProductAnalytics):SupplierProductRanking|undefined => {
  const metrics=(['ML_CLASSICO','ML_PREMIUM','SHOPEE','AMAZON'] as ScenarioKey[]).flatMap(key=>product.scenarios[key]?[product.scenarios[key]!]:[]);
  if(!metrics.length)return undefined;
  return {product,averageValue:metrics.reduce((sum,metric)=>sum+metric.marginValue,0)/metrics.length,averagePercent:metrics.reduce((sum,metric)=>sum+metric.marginPercent,0)/metrics.length};
};
...
export async function loadSupplierAnalytics(supplierId:string, region:AnalyticsRegion):Promise<SupplierAnalytics|null> {
  ...
  for(const c of calculations){ const key=scenario(marketMap.get(c.marketplace_id)??"",c.listing_type); if(!key)continue; const input=obj(c.input_snapshot),regional=obj(obj(c.results)[region] as Json),metric={price:Number(c.sale_price),shipping:Number(c.shipping_cost),marginValue:Number(regional.contributionMarginValue??0),marginPercent:Number(regional.contributionMarginPercent??0),createdAt:c.created_at,commission:Number(regional.marketplacePercentageFee??0),fixedFee:Number(regional.marketplaceFixedFee??0),rebate:Number(regional.marketplaceRebate??input.marketplaceRebateValue??0)}; const mapKey=`${c.product_id}:${key}`;if(!latest.has(mapKey))latest.set(mapKey,metric); }
  const rows=(products??[]).map(p=>({id:p.id,sku:p.sku,childSkus:childrenByProduct.get(p.id)??[],name:p.name,manufacturerCode:p.manufacturer_code,active:p.active,cost:Number(p.cost),pending:pendingSet.has(p.id),scenarios:Object.fromEntries((["ML_CLASSICO","ML_PREMIUM","SHOPEE","AMAZON"] as ScenarioKey[]).flatMap(k=>{const v=latest.get(`${p.id}:${k}`);return v?[[k,v]]:[]})),pricingHistory:[]} as SupplierProductAnalytics));
